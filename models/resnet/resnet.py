...
class ResNetV2(nn.Module):
    ...
    def __init__(self, config):
        ...
        block_units = BLOCK_UNITS[config['arch']]
        wf = config['width_factor']  # shortcut 'cause we'll use it a lot.
        head_size = config['output_size']
        ...
        if config['weights_file']:
            self.load_from(np.load(config['weights_file']))

    def forward(self, x):
        pre_pooled_feats = self.body(self.root(x))
        # Need to perform the final linear layer prior to pooling
        pre_pooled_feats = self.head[3](nn.Sequential(*list(self.head.children())[:-2])(pre_pooled_feats))
        output_feat = self.head[2](pre_pooled_feats)
        return output_feat, pre_pooled_feats.flatten(start_dim=2).transpose(2, 1)
        ...
        with torch.no_grad():
            ...
            if self.zero_head:
                ...
                try:
                    ...
                    nn.init.xavier_uniform_(self.head.conv.weight)
                    nn.init.zeros_(self.head.conv.bias)
